...
Condition
    ...
    - 작동
        1. 현재 위치를 청소
        2. 현재 위치에서, 주변 4칸중 청소되지 않은 빈 칸이 있는 경우
        2-1. 반시계 방향으로 90도 회전
        2-2. 바라보는 방향에서, 앞쪽 칸이 청소되지 않은 빈칸이라면 한칸 전진
        2-3. 1번으로 돌아감 
        3. 현재 위치에서, 주변 4칸중 청소되지 않은 빈 칸이 없는경우
        3-1. 바라보는 방향을 유지하고, 후진할 수 있다면 후진
             다시 1번으로
        3-2. 후진할 수 없다면 작동 종료
